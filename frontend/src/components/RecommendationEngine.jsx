    ...
    <div className="bg-white rounded-lg shadow-sm">
      ...
      <div className="p-4 border-b border-gray-200">
        <div className="flex items-center justify-between mb-3">
          ...
          <span className="text-xs text-gray-500">{recommendations.length} routes</span>
        </div>
        ...
        {/* Change Indicator */}
        {newRoutes.length > 0 && (
          <div className="bg-green-50 border border-green-300 rounded px-3 py-2 text-xs text-green-800 font-semibold">
            ✨ {newRoutes.length} new {newRoutes.length === 1 ? 'route' : 'routes'} found!
          </div>
        )}
      </div>
      ...
      <div className="p-4 bg-gray-50 border-b border-gray-200 space-y-3">
        ...
        <div>
          ...
          <select
            ...
            className="w-full text-sm px-3 py-1.5 border border-gray-300 rounded focus:ring-2 focus:ring-blue-500"
          >
                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex items-start space-x-2 flex-1">
                      <span className={`flex-shrink-0 w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold text-white ${
                        isNew ? 'bg-green-500' : index === 0 ? 'bg-yellow-500' : 'bg-blue-500'
                      }`}>
                        {index + 1}
                      </span>
      ...
      <div className="p-3 bg-gray-50 border-t border-gray-200 text-xs text-gray-600">
        <div className="flex items-center justify-between">
          <span>Using FAISS {strategy.includes('mmr') && '+ MMR'}</span>
